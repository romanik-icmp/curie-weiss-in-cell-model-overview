...
\begin{document}
	...
	The cell model was introduced and described in~\cite{KozitskyKozlovskiiDobush2018book,KozitskyKozlovskiiDobush2020}. This model possesses an exact solution in the case of Curie-Weiss interaction. The exact asymptotic expression for the grand partition function was obtained in~\cite{KozitskyKozlovskiiDobush2020}.
	...
	\begin{equation}
		...
		\right.
	\end{equation}
	In~\cite{KozitskyKozlovskiiDobush2020}, the notation $\abs{\gamma}$ was used to denote the number of points (particles) in configuration $\gamma$, thus $\abs{\gamma} \equiv N$.
	
	\begin{mdframed}[linecolor=black,linewidth=1pt,leftline=true]
	If the notation from~\cite{hansen2013theory} is followed, the previous formulas can be rewritten as follows. The space coordinate of the $i$-th particle is denoted by $\vb{r}_i$, and thus the configuration of $N$ particles is defined by $\gamma = \{\vb{r}^N\}$, where $\vb{r}^N \equiv \vb{r}_1, ..., \vb{r}_N$. The interaction energy is expressed as (cf.~\cite[eq.~(2.5.16)]{hansen2013theory})
	\begin{eqnarray*}
	...
	\begin{equation*}
		...
		\right.
	\end{equation*}
	Unlike~\cite[eq.~(2.5.16)]{hansen2013theory}, we do not exclude term with $i = j$ in the expression for $W_{N_v}$ due to the self-interaction.
	\end{mdframed}
	...
	\begin{equation*}
		J_2 > J_1
	\end{equation*}
	to satisfy such inequality~\cite{KozitskyKozlovskiiDobush2020,Ruelle1970}
	\begin{equation}
	...
	\textbf{The grand partition function} is expressed as follows~\cite[eqs.~(2.4.6) and~(2.3.13)]{hansen2013theory}
	\begin{equation*}
	...
	\begin{mdframed}[linecolor=black,linewidth=1pt,leftline=true]
		Alternatively, in notation from~\cite{hansen2013theory}, the grand partition function is expressed as [cf. eqs.~(2.4.6) and~(2.3.13)]
		\begin{equation}
	...
	\end{eqnarray*}
	For any dimensional quantity $Q$, it is convenient to denote its reduced counterpart by $Q^*$. To maintain consistency with other thermodynamic quantities, it may be beneficial to use $\beta^*$ instead of $p$ for the reduced inverse temperature (cf.~\cite{RioEtAl2013}).
	...
	\begin{mdframed}[linecolor=black,linewidth=1pt,leftline=true]
		...
		\end{equation}
	\end{mdframed}
	Equations~\eqref{eq:gpf1} and~\eqref{eq:gpf1_alt} can be compared with~\cite[(2.5)]{KozitskyKozlovskiiDobush2020}.
