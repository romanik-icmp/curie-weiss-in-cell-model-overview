...
\begin{document}
	...
	\end{equation}
	The typical behavior of $E(T^*,\mu^*;y)$ as a function of $y$ in the single-phase domain is illustrated in Fig.~\ref{fig:E0_vs_y}
	\begin{figure}[htbp]
	...
	Let $\bar{y}$ denote the point of global maximum of $E(T^*,\mu^*;y)$ with respect to $y$ at given temperature and chemical potential. Then it should obey the following equation
	\begin{equation}
		\label{def:E1}
	...
	As $K_0$, $K_1$, and $T^*$ all take on strictly positive values, the solution $\bar{y}$ to equation~\eqref{eq:bar_y} is also strictly positive.
	Typical behavior of $E_1(T^*,\mu^*;y)$ as a function of $y$ in the single-phase domain is illustrated in Fig.~\ref{fig:E1_vs_y}.
	
	\textbf{Definition}. We say that $(T^*, \mu^*)$ belongs to a single-phase domain if $E(T^*,\mu^*;y)$ has a unique global maximum $\bar{y} \in \mathbb{R}^{+}$ such that
	\begin{equation}
		\label{def:E2}
		E_2(T^*, \mu^*;\bar{y}) := \frac{\partial^2}{\partial y^2} E(T^*,\mu^*;y)\bigg|_{y=\bar{y}} < 0.
	\end{equation}
	...
	\begin{equation}
		\Xi(T^*,\mu^*) = [-T^*/ E_2(T^*,\mu^*;\bar{y})]^{1/2} \exp[N_v E(T^*,\mu^*;\bar{y})].
	\end{equation}
	Substituting this into~\eqref{def:eos}, one obtains
	\begin{equation}
		P V = k_{\rm B}T \left[-\frac{1}{2} \ln(-\frac{E_2(T^*,\mu^*;\bar{y})}{T^*}) + N_v E(T^*,\mu^*;\bar{y})\right].
	\end{equation}
	...
	\begin{equation}
		\frac{P V}{k_{\rm B}T N_v} = E(T^*,\mu^*;\bar{y}),
	\end{equation}
	or
	\begin{equation}
		\label{eos:reduced}
		P^*(T^*,\mu^*) = T^* E(T^*,\mu^*;\bar{y}).
	\end{equation}
	We have obtained an expression for the pressure $P^*$ as a function of temperature $T^*$ and chemical potential $\mu^*$, $P = P(T^*, \mu^*)$. We also remember that the quantity $\bar{y}$, which is selected from the condition to maximize $E$ at given $T^*$ and $\mu^*$, is a function of $T^*$ and $\mu^*$ itself, $\bar{y} = \bar{y}(T^*,\mu^*)$. Since we can't obtain an analytic solution to~\eqref{eq:bar_y}, we'll use numerical methods to approximate $\bar{y}$ and then illustrate the results graphically.
