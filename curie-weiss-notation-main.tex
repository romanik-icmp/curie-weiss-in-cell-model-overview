...
\begin{document}
	...
	The cell model was introduced and described in~\cite{KKD2018,KKD2020}. This model possesses an exact solution in the case of Curie-Weiss interaction. The exact asymptotic expression for the grand partition function was obtained in~\cite{KKD2020}.
	...
	\begin{equation}
		...
		\right.
	\end{equation}
	In~\cite{KKD2020}, the notation $\abs{\gamma}$ was used to denote the number of points (particles) in configuration $\gamma$, thus $\abs{\gamma} \equiv N$.
	
	\begin{mdframed}[linecolor=black,linewidth=1pt,leftline=true]
	If the notation from~\cite{HansenMcDonald2013} is followed, the previous formulas can be rewritten as follows. The space coordinate of the $i$-th particle is denoted by $\vb{r}_i$, and thus the configuration of $N$ particles is defined by $\gamma = \{\vb{r}^N\}$, where $\vb{r}^N \equiv \vb{r}_1, ..., \vb{r}_N$. The interaction energy is expressed as (cf.~\cite[eq.~(2.5.16)]{HansenMcDonald2013})
	\begin{eqnarray*}
	...
	\begin{equation*}
		...
		\right.
	\end{equation*}
	Unlike~\cite[eq.~(2.5.16)]{HansenMcDonald2013}, we do not exclude term with $i = j$ in the expression for $W_{N_v}$ due to the self-interaction.
	\end{mdframed}
	...
	\begin{equation*}
		J_2 > J_1
	\end{equation*}
	to satisfy such inequality~\cite{KKD2020,Ruelle1970}
	\begin{equation}
	...
	\textbf{The grand partition function} is expressed as follows~\cite[eqs.~(2.4.6) and~(2.3.13)]{HansenMcDonald2013}
	\begin{equation*}
	...
	\begin{mdframed}[linecolor=black,linewidth=1pt,leftline=true]
		Alternatively, in notation from~\cite{HansenMcDonald2013}, the grand partition function is expressed as [cf. eqs.~(2.4.6) and~(2.3.13)]
		\begin{equation}
	...
	Alternatively
		...
		\end{equation}
	\end{mdframed}
	Equations~\eqref{eq:gpf1} and~\eqref{eq:gpf1_alt} can be compared with~\cite[(2.5)]{KKD2020}.
